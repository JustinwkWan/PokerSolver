...
// Maximum number of players supported (2-6, targeting 6-max).
...
struct GameState {
    ...
    int num_players;   // number of players (2-6)

    // ── Cards ─────────────────────────────────────────────────────────────
    std::array<std::array<Card, 2>, kMaxPlayers> hole;  // hole[player][0..1]
    std::array<Card, 5>                          board; // kNoCard until dealt
    int                                          num_board_cards;

    // ── Pot / stacks ──────────────────────────────────────────────────────
    int pot;                        // chips collected into pot from PREVIOUS streets
    int stacks[kMaxPlayers];        // current stacks (chips NOT yet in pot or bet)
    int street_bet[kMaxPlayers];    // chips each player has committed THIS street

    // ── Betting state ─────────────────────────────────────────────────────
    Street street;
    int    actor;            // whose turn
    int    button;           // dealer seat
    bool   folded[kMaxPlayers];
    bool   all_in[kMaxPlayers];
    bool   has_decided[kMaxPlayers];   // has made ≥1 real decision this street
    ...
    // num_players defaults to 2 for backward compatibility.
    ...
    // HU backward-compat overload:
    GameState withHoleCards(Card p0c1, Card p0c2, Card p1c1, Card p1c2) const;
    // N-player version: hole_cards[player] = {card0, card1}
    GameState withHoleCards(const std::array<std::array<Card, 2>, kMaxPlayers>& cards) const;
    ...
    int potSize() const {
        int total = pot;
        for (int i = 0; i < num_players; ++i) total += street_bet[i];
        return total;
    }

    // Number of non-folded players
    int numNonFolded() const;

    // Next active (non-folded, non-all-in) player after `from`, scanning circularly.
    ...
    // ── Terminal payoffs ──────────────────────────────────────────────────
    // If someone folded (only one remains), returns the winner index.
